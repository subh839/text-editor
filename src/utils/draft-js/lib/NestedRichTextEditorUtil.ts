...
var ContentBlockNode = require('draft-js/lib/ContentBlockNode');

var DraftModifier = require('./DraftModifier');

var DraftTreeOperations = require('draft-js/lib/DraftTreeOperations');

var EditorState = require('draft-js/lib/EditorState');

var RichTextEditorUtil = require('draft-js/lib/RichTextEditorUtil');

var adjustBlockDepthForContentState = require('draft-js/lib/adjustBlockDepthForContentState');

var generateRandomKey = require('draft-js/lib/generateRandomKey');
...
var NestedRichTextEditorUtil = {
  ...
  onBackspace: function onBackspace(editorState) {
    ...
    var withoutBlockStyle = NestedRichTextEditorUtil.tryToRemoveBlockStyle(
      editorState
    );
  ...
  toggleBlockType: function toggleBlockType(editorState, blockType) {
    ...
    var isMultiBlockSelection =
      selection.getAnchorKey() !== selection.getFocusKey();
    var isUnsupportedNestingBlockType = NESTING_DISABLED_TYPES.includes(
      blockType
    );
    var isCurrentBlockOfUnsupportedNestingBlockType = NESTING_DISABLED_TYPES.includes(
...
var onUntab = function onUntab(blockMap, block) {
  ...
  if (blockIndex === 0 || blockIndex === existingChildren.count() - 1) {
    ...
    !(nextSiblingKey != null)
      ? process.env.NODE_ENV !== 'production'
        ? invariant(false, 'block must have a next sibling here')
        : invariant(false)
      : void 0;
    ...
    blockMap = blockMap.map(function(block) {
      return nextChildren.includes(block.getKey())
        ? block.merge({
  ...
  if (parentKey != null) {
    ...
    while (_parent != null) {
      ...
      !(firstChildKey != null)
        ? process.env.NODE_ENV !== 'production'
          ? invariant(false, 'parent must have at least one child')
          : invariant(false)
        : void 0;
  ...
  if (childWasUntabbed && parentKey != null) {
    ...
    var prevSiblingKey =
      _parent2 != null // parent may have been deleted
        ? _parent2.getPrevSiblingKey()
        : null;
